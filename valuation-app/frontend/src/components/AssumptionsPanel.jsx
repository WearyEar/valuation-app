        ...
        <div className="flex items-center gap-1.5">
          <input
            ...
            className="w-20 bg-gray-800 border border-gray-700 rounded px-2 py-0.5 text-xs
                       text-right text-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
            value={display}
            step={0.1}
...
function NumRow({ label, hint, value, step = 0.01, min, max, onChange }) {
  return (
    <div className="flex justify-between items-center gap-4">
      ...
      <input
        type="number"
        className="w-24 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-xs
                   text-right text-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
        value={value}
        step={step}
        min={min}
    ...
    <div className="card">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-gray-200">Assumptions</span>
          {recalculating && (
      ...
      {open && (
        <div className="mt-5 space-y-6">
          ...
          <div className="flex gap-2 flex-wrap">
            <div className="bg-gray-800 rounded-lg px-3 py-2 flex flex-col items-center gap-0.5 flex-1 min-w-24">
          ...
          <div className="space-y-4">
            ...
            <SliderRow
              label="Revenue Growth Rate (Yrs 1–5)"
              hint="Fades linearly to stable growth by year 10"
              value={assumptions.revenue_growth_rate}
              min={-0.20} max={0.50} step={0.001}
              onChange={(v) => update('revenue_growth_rate', v)}
            />
            <SliderRow
              label="Stable / Terminal Growth Rate"
              hint="Should approximate long-run nominal GDP growth (≈ risk-free rate)"
              value={assumptions.stable_growth_rate}
          ...
          <div className="space-y-4">
            ...
            <SliderRow
              ...
              hint="Trailing EBIT / Revenue"
              value={assumptions.current_operating_margin}
              min={-0.30} max={0.60} step={0.001}
              onChange={(v) => update('current_operating_margin', v)}
            />
            <SliderRow
              label="Target Operating Margin"
              hint="Converges from current over 5 years (sector or analyst target)"
              value={assumptions.target_operating_margin}
              min={-0.10} max={0.60} step={0.001}
          ...
          <div className="space-y-4">
            ...
            <SliderRow
              ...
              hint="10-year US Treasury yield"
            ...
            <SliderRow
              ...
              hint="Damodaran's implied ERP for US market"
              value={assumptions.equity_risk_premium}
              min={0.02} max={0.10} step={0.001}
              onChange={(v) => update('equity_risk_premium', v)}
            />
            <SliderRow
              label="Beta (Levered)"
              hint="Re-levered from Damodaran's sector unlevered beta"
              value={assumptions.beta / 4}           /* normalise 0–1 for slider */
              min={0.05} max={0.75} step={0.0025}
              onChange={(v) => update('beta', v * 4)}
            />
            {/* Show actual beta value clearly */}
            <p className="text-xs text-gray-500 -mt-2">
              Beta: <span className="text-gray-300 font-mono">{assumptions.beta.toFixed(2)}</span>
            </p>
            <SliderRow
              label="Pre-tax Cost of Debt"
              hint="Interest expense / total debt"
            ...
            <NumRow
              ...
              value={(assumptions.tax_rate * 100).toFixed(1)}
          ...
          <div className="space-y-4">
            ...
            <NumRow
              ...
              hint="Revenue / Invested Capital (drives reinvestment)"
              value={assumptions.sales_to_capital_ratio.toFixed(2)}
